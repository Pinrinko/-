...
void DynamicPriorityScheduler::clear() {
    while (ready) {
        DynamicPriorityPCB* p = ready;
        ready = p->link;
        delete p;
    }
}

void DynamicPriorityScheduler::sort(DynamicPriorityPCB* new_p) {
    DynamicPriorityPCB* first, * second;
    int insert = 0;
...
void DynamicPriorityScheduler::setProcessList(const QList<QPair<QString, QPair<int, int>>>& plist) {
    clear();
    for (const auto& pair : plist) {
        DynamicPriorityPCB* p = new DynamicPriorityPCB;
        p->name = pair.first;
...
QString DynamicPriorityScheduler::display(DynamicPriorityPCB* running_p) {
    QString res;
...
QString DynamicPriorityScheduler::runSimulation() {
    QString output;
    int h = 0;
    ...
    while (ready != nullptr) {
        ...
        DynamicPriorityPCB* current = ready;
        ready = current->link;
